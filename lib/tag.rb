...
class Tag
  ...
  def self.where(bookmark_id:)
    result = DatabaseConnection.query("SELECT bookmark_tags.id, content FROM bookmark_tags INNER JOIN tags ON tags.id = bookmark_tags.tag_id WHERE bookmark_tags.bookmark_id = '#{bookmark_id}';")
    result.map do |tag|
